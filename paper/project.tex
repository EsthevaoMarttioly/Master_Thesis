...
- \frac{(1-\tau_\ell) w_t L_t}{\mu_w h_t}  \right]
...
$\ln a \in [\ln \underline a, \ln \bar a] = [\mAmin, \mAmax]$ with $N_a = \mNA$ points, 
and a log-normal sector productivity grid $\ln \theta^s \sim N(\mu_s, \sigma_s^2)$,
that is discretized by a Gauss-Hermite quadrature with $N_\theta = \mNT$
nodes \parencite{meghir2015wages}.
...
the HANK literature \parencite{kaplan2018monetary,auclert2025fiscal}\footnote{
    ...
    \textcite{basu1997returns}.},
and $\kappa = \kappa_w = \mKappa$, consistent with \textcite{hazell2022national} estimates.

Taylor Rule is pretty standard, with $\phi=\mPhi$ and $r^* = \mRstar$, consistent
with a $4\%$ annual real interest rate for Brazilian economy
...
$\phi_B = \mPhiB$ so that the average time to pay off the debt is 10 years,
as in \textcite{auclert2025fiscal}. Wage tax is an average of all contributions
levied on the payroll, $\tau_\ell = \mTauL$.
...
such as $\gamma$, $\mu$, and $\kappa_w$. This parameters might have minor
implications in the results, but it is expected that the model continues
...
$\int_F e_{i,t} d\Lambda = L_t / h_t$, it is possible to find that
\[
\theta_w (\pi_t^w - \bar \Pi^w) \pi_t^w
= \varepsilon_w \left[ \frac{v'(h_t)}{u'(\tilde C_t)}
- \frac{\varepsilon_w-1}{\varepsilon_w} (1-\tau_\ell) \frac{w_t L_t}{h_t} \right]
...
- (1-\tau_\ell) \frac{w_t L_t}{h_t} \frac{\varepsilon_w-1}{\varepsilon_w} \right]
+ \bar \beta \ln (1+\pi_{t+1}^w).
\]
