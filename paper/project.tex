...
\usepackage{csquotes}
\usepackage{float}
% Use bibtex backend to avoid requiring Biber in the build environment
...
$\beta_i\in\{ \beta_L, \beta_H \}$, and we fix $\omega_I$
...
$\theta^s_{i,t}$ which their wage depends on, re-drawn on each new offer,
with $\E\!\left[\exp\{\theta^F_{i,t}\}\right] = 1.0$.

Formal workers earn labor income
$(1-\tau_{\ell}) \theta_{i,t}^F w_t e_{i,t} h_{i,t}$, while informal workers
earn $\theta_{i,t}^I w_t e_{i,t} h_{i,t}$, where $w_t$ is the baseline wage,
$\tau_{\ell}$ is a fixed labor income tax rate. Let $y_t(s,e,\theta)$ be this
non-financial income,
\[
    y_t(s,e,\theta) = \begin{cases}
        (1-\tau_\ell) \theta^F w_t h_t e, &\text{if formal;} \\
        \theta^I w_t h_t e, \quad &\text{if informal;} \\
        0, &\text{if unemployed}.
    \end{cases}
\]

We assume that each individual may be eligible to receive
\textit{Bolsa Família} (BF), depending on their sector and wage. We
model the probability of receipt based on the data, in such a way that
unemployed workers receive with a fixed probability, while informal and
formal workers receive BF with a probability that decreases as income rises,
\[
\P[i\text{ is eligible}|s,y] = \frac{\phi_s}{1 + (y/\bar y_s)^{1/\sigma_s}},
\]
where $\phi_s$ is the take-up at zero earnings, $\bar y_s$ is the half-coverage
threshold, and $\sigma_s$ is the smoothness of eligibility, for each $s\in\{F,I\}$.
For any income level, an informal worker is more likely to receive the benefit,
$\phi_I > \phi_F$. The total number of beneficiaries is $\text{BF}_t$ and the
individual benefit is
$T_{i,t}(s,y) = T_t \cdot \mathbbm{1}_{i,t}\{\text{eligible}\}(s,y)$.
...
\[
c_{i,t} + a_{i,t} = (1+r_t) a_{i,t-1} + y_{i,t}(s,e,\theta) + T_{i,t}(s,y) + d_{i,t} + \tau_t.
\]
...
\[
\psi \left( h \right)^{1/\varphi} = \theta w e
\quad \Leftrightarrow \quad h^I(e)
= \left[ \frac{\theta w e}{\psi} \right]^\varphi.
\]
...
probability $\pi_s$ if employed or $\pi_s^U$ if unemployed.
The household accepts the offer with the higher value, smoothed by additive
idiosyncratic preference shocks explained later.
...
\begin{align*}
    ...
    &\quad\qquad + \pi_F (1-\pi_I) \max\!\left\{V_{t+1}^U(.),
            \E_\theta V_{t+1}^F(.)\right\} \\
    &\quad\qquad + (1-\pi_F)\pi_I \max\!\left\{V_{t+1}^U(.), 
            \E_\theta V_{t+1}^I(.)\right\} \\
    &\quad\qquad + \pi_F \pi_I \max\!\left\{V_{t+1}^U(.), 
            \E_\theta V_{t+1}^I(.), \E_\theta V_{t+1}^F(.)\right\}
    \bigg] \Bigg\} \\
...
$\alpha_s$, such that $\alpha_F+\alpha_I+\alpha_U=1$.
Let $\Lambda_t(s,\beta,e,a)$ be the household distribution over a given
period of time.
...
standard in Neo-Keynesian's literature, with a quadratic adjustment cost
\[
\Psi_t(\pi_t) = \frac{\mu}{\mu-1} \frac{1}{2\kappa}
\left[ \log (1+\pi_t) \right]^2 Y_t.
\]

Firms' dividends are taxed at the same rate $\tau_{\ell}$ as labor income, so
$d_t = (1-\tau_{\ell}) (Y_t - w_t L_t) - \Psi_t(\pi_t)$, where $L_t = \E[l_{j,t}]$
is the aggregate formal labor. Such as \textcite{hagedorn2019fiscal},
...
- \frac{(1-\tau_\ell) w_t L_t}{\mu_w h_t F_t}  \right]
+ \bar \beta \ln (1+\pi_{t+1}^w).
\]
where $L_t / (h_t F_t) = \E(e_{i,t} | F)$, since the union's members are the formal
workers only.
...
issuing new debt. The deficit-financed policy sets a path of $T_t$ and $\tau_t$
and adjusts debt, keeping the nominal interest rate fixed $i_t = \bar i$.
We assume that the government repays its debt in the long run by reducing
transfer (when $\tau_t > 0$) or increasing taxes (when $\tau_t < 0$), with
elasticity $\phi_B$,
\[
\tau_t = \bar \tau - \phi_B \cdot (B_{t-1} - \bar B).
\]

Or, in another words, using \textcite{auclert2025fiscal} notation, the debt
persistency is defined by
\[
dB_t = (1+r_t) dB_{t-1} + d\tau_t = (1 + r - \phi_B) dB_{t-1} = \rho_B dB_{t-1},
\]
where $\rho_B$ is the debt persistency, which we calibrate so that debt is paid off
on average within 10 year, so $\rho_B = 0.975$ and $\phi_B = \mPhiB$.
...
(Market Clearing Conditions). First, the asset market reaches
...
\begin{align*}
    ...
    &L_t^I = \int \mathbbm{1}\{s_{i,t}=I \} \cdot \theta^I_{i,t} \cdot
            e_{i,t} \cdot h_{i,t} \; d\Lambda_t(s,\beta,e,a);
\end{align*}
and the goods market clear when $Y + Y^I = C + \Psi_t$. We set the goods market to
clear by Walras' Law. Lastly, the BF benefit is given to all eligible households,
\[
...
This panel is interesting to match the sector transitions and the worked hours
and wages, for both formal and informal households. However, since it is a
survey, it might have under-reporting of informal households.
...
from UBS, and the Wealth Inequality Statistics were obtained from the 2024 Wealth
Inequality Database\footnote{
    Available in \href{https://wid.world/world/}{WID World}.}.
\textit{Bolsa Família} data is available in \textit{Cadastro Único}
(CadÚnico), in VIS Data\footnote{
...
a platform with several statistics about the benefit.

Additionally, Annual PNAD-C has information about whether a individual
is beneficiary of the benefit. We identify each household and consider every
individual living in a beneficiary household as receiving the benefit.
Among those, we calculate the mean reported benefit $T$ and the mean share of
beneficiaries.

\input{bf_size.tex}
\input{bf_coverage.tex}

The rule to receive BF is based on a R\$ 218 threshold for the family income
per capita, therefore there are some formal individuals receiving the benefit.
Also, CadÚnico normally delay to remove the benefit for non-eligible individuals,
which can explain why some formal individuals receive BF.

In order to construct quarter-to-quarter transition probabilities across the
three sectors $s\in\{F,I,U\}$, we reconstruct the PNAD-C rotating panel using
\textcite{osorio2022montagem}, which estimates the dwelling and individual
identifiers in the survey. The estimation of the pandemic years is not reliable.

The formality status is defined as (i) formal workers have signed work card,
are militaries, or employers, and (ii) informal workers do not have a signed
work card, are self-employed, or unpaid family. We only consider household
in the labor force, dropping those who are non-participating, retired, or
students, to keep the empirical states aligned with the model's.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{shares_timeseries.png}
    \caption{Share of Formal, Informal, and Unemployed Households}
    \label{fig:shares_timeseries}
\end{figure}


\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\linewidth]{transition_timeseries.png}
    \caption{Sector Transitions over time}
    \label{fig:transition_timeseries}
\end{figure}


\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{calibration_timeseries.png}
    \caption{Wage and Worked Hours for each Sector}
    \label{fig:calibration_timeseries}
\end{figure}


\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\linewidth]{wage_distribution.png}
    \caption{Wage Distribution for each Sector}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{bf_coverage_by_wage.png}
    \caption{Share of Households Receiving Benefit by Wage Level}
    \label{fig:bf_coverage_by_wage}
\end{figure}


\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{bf_wage.png}
    \caption{Wage Distribution by Benefit Status}
    \label{fig:bf_wage}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{bf_eligibility.png}
    \caption{Familiar Wage Distribution by Sector, for Beneficiaries}
    \label{fig:bf_eligibility}
\end{figure}
...
among sectors using \textcite{iskhakov2017endogenous}, with taste shocks.
...
100 quarters (or 25 years). We externally calibrate $\beta_H$ to balance the
asset market and $\pi_F$, $\pi_I$, $\pi_{UF}$, $\pi_{UI}$ to match the
sectoral probabilities. Also, we internally calibrate parameters using the
Simulated Method of Moments (SMM) by a Global Search on a coarse grid, followed
by a local polish using the Nelder-Mead algorithm. The external calibration is
shown in table \ref{tab:ext_calibration} and the endogenous calibration is
in \ref{tab:int_calibration}.
...
This is approximated by a finite Markov chain with $N_e = \mNE$ grid points
\parencite{rouwenhorst1995asset}. We internally calibrate $\rho_e$ and $\sigma_e$
to match the autocorrelation of wages in PNAD-C
$\rho_1^y := \text{Corr}(\ln y_t, \ln y_{t-1})$ and the observed wage distribution.

We also assume a log-spaced grid for assets
$\ln a \in [\ln \underline a, \ln \bar a] = [\mAmin, \mAmax]$ with $N_a = \mNA$
points, sufficient to cover the top distribution, and a log-normal sector
productivity grid $\ln \theta^s \sim N(\mu_s, \sigma_s^2)$, that is discretized
by Equiprobable Bins with $N_\theta = \mNT$ nodes \parencite{meghir2015wages}.

Each household has a $q = \mQ$ probability of drawing a new discount factor
...
in literature $\gamma=\mEis$\footnote{
    EIS $\gamma=\mEis$ is consistent with micro estimates surveyed by
    \textcite{valickova2015financial}.}.
The disutility of labor $\psi = \mPsi$ is calibrated to normalize formal
hours $h_F = \mHF$.
% At last, $\varphi = \mVarphi$ is chosen to approximate
% informal worked hours to PNAD-C Data, in average (model = $0.891$, data = $0.867$)
% and in standard error (model = $0.35$, data = $0.34$).
Finally, the preference shock smoothing parameter is set to $\sigma_S = \mSig$,
so as to match the income growth of workers switching between the formal and informal
sectors, $\Delta_{FI} = \E[\Delta\ln y_{t+1} | F \to I] = \mDatDwFI$ and
$\Delta_{IF} = \E[\Delta\ln y_{t+1} | I \to F] = \mDatDwIF$.

The patient's discount factor $\beta_H = \mBetaHigh$, balances the asset market.
We estimate the impatient's discount factor, $\beta_L = \mBetaLow$, and the share
of impatient households, $\omega_I = \mOmegaI$, to match the Top 10\% Share of Wealth
($\mDatTopOneZero$, from World Inequality Database) and the share of Hand-to-Mouth
households ($\mDatHtm$)\footnote{
    HtM share consistent with the estimates of \textcite{costa2025fiscal} and
    \textcite{galveasuncertain}.},
letting the Wealth Gini Coeeficient ($\mDatGini$, from UBI Global Report) and th
Top 1\% Share of Wealth ($\mDatTopOne$) untargeted.

The layoff probabilities are equal to the transition probabilities in PNAD-C,
$\delta_F = \P[\text{F}\to\text{U}] = \mDeltaF$ and
$\delta_I = \P[\text{I}\to\text{U}] = \mDeltaI$. Further, the job offer
probabilities are calibrated to match sector sizes $s\in\{F,I,U\}$ in
equilibrium, as shown in Table \ref{tab:transitions}. The taste preferences
smoothness is governed by $\sigma_S = \mSig$.

\input{transition_table.tex}

We internally calibrate moments of $\theta$ to match five quantiles
$\{0.10, 0.25, 0.50, 0.75, 0.90\}$ of the observed wage distribution for
formal and informal workers, with respect to $\E(y^F)$, which yields
$\mu_I = \mMuI$, $\sigma_F = \mSigmaF$, and $\sigma_I = \mSigmaI$,
after normalizing $\E(\theta^F) = 1.0$.

The average wage is normalized $w=\mW$, so that we can reference the model as a
percentage of the Wage Bill. That was prefered rather than normalizing
the GDP, because there is no capital in this model, therefore the wage if a better
measure of the economy's size. We defined $\mu = \mMu$, which is standard in
the HANK literature \parencite{kaplan2018monetary,auclert2025fiscal}\footnote{
    Markup $\mu=\mMu$ is also consistent with the estimates of
    \textcite{basu1997returns} for US. \textcite{de2021quantifying} show that
    the Brazilian markup is similar to the US.},
and $\kappa = \kappa_w = \mKappa$, similar to \textcite{kaplan2018monetary}
and consistent with an average duration of 4 quarters for readjustment.

Taylor Rule is pretty standard, with $\phi=\mPhi$ and $r^* = \mRstar$, consistent
with a $5\%$ annual real interest rate for Brazilian economy
\parencite{dos2024natural}. The Government debt is set to $\mDatBGdp$ of yearly
GDP, consistent with Brazilian National Treasure data. Also, we set
$\phi_B = \mPhiB$ so that the half-live of the debt is 5 years, slower than
\textcite{auclert2025fiscal} and consistent with \textcite{campos2019alert}.
Wage tax is based on the average formal worker contribution, $\tau_\ell = \mTauL$.

We calibrated the \textit{Bolsa Família} transfer size $T = \mTr$ to match the
\textit{Bolsa Família} expenditure as a percentage of the Wage Bill,
$\text{BF} \cdot T / W$ from Brazilian economy. In order to keep the internal
consistency of the model, we consider $T$ as the average transfer received only
by indivuals in labor force, disconsidering children and elderly. On the other
hand, we calibrated the benefit's eligibility parameters to match PNAD-C data
(Figure \ref{fig:bf_coverage_by_wage}), with a Weighted Non-Linear Least
Square (Weighted NLS) estimation.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{income_distribution.png}
...
\begin{figure}[H]
    ...
    \caption{iMPC: Partial Equilibrium Profile}
    \label{fig:impc}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{irf_financing.png}
    \caption{Response to a Shock in $T$ by Financing Rules}
    \label{fig:irf_financing}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Response to a Shock in $T$ by Channel}
...
\begin{figure}[H]
    ...
    \caption{Response to a Shock in $T$: Instant vs Antecipated}
    \label{fig:irf_antecipated}
\end{figure}

\input{cumulative_response.tex}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{irfm_decomposition.png}
    \caption{Response to an increase in $i$ by Channel}
    \label{fig:irfm_decomposition}
\end{figure}


\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{irfm_bf_vs_nobf.png}
    \caption{Response to an increase in $i$: With and Without CCT}
    \label{fig:irfm_bf_vs_nobf}
\end{figure}

\input{monetary_cumulative.tex}
...
such as $\gamma$, $r^*$ (4\% and 6\%), and $\phi$ (2.0). This parameters might
have minor implications in the results, but it is expected that the model continues
...
to a standard separable utility in the form of $u(c) - v(h)$, which must
...
$\int_F e_{i,t} d\Lambda = L_t / h_t$ and $\int_F d\Lambda = F_t$, it is possible
to find that
\[
\theta_w (\pi_t^w - \bar \Pi^w) \pi_t^w
= \varepsilon_w \left[ \frac{F_t h_t}{L_t} \frac{v'(h_t)}{u'(\tilde C_t)}
- \frac{\varepsilon_w-1}{\varepsilon_w} (1-\tau_\ell) w_t \right]
+ \bar \beta \theta_w (\pi_{t+1}^w - \bar \Pi^w) \pi_{t+1}^w \frac{h_{t+1}}{h_t},
\]
where $L_t/(h_t F_t) = \E(e_{i,t} | F)$. We can linearize around the steady
state $\pi_t^w = \bar \Pi^w$ and take $h_{t+1} = h_t$, since there is no population
nor technological growth. Thus,
\[
\ln (1+\pi_t^w)
= \frac{\varepsilon_w}{\theta_w} \frac{F_t h_t}{L_t}
\left[ \frac{v'(h_t)}{u'(\tilde C_t)}
- (1-\tau_\ell) \frac{w_t L_t}{h_t F_t} \frac{\varepsilon_w-1}{\varepsilon_w} \right]
+ \bar \beta \ln (1+\pi_{t+1}^w).
\]

Therefore, defining $\kappa_w = (\varepsilon_w/\theta_w) \cdot F h / L$ at the
steady state and $\mu_w = \varepsilon_w/(\varepsilon_w-1)$, the wage Phillips
curve is
\[
\ln (1+\pi_t^w)
= \kappa_w \left[ \psi \cdot h_t^{1/\varphi} \cdot \tilde C_t^{1/\gamma}
