...
(CadÚnico), in VIS Data, a platform with several statistics about the benefit.
...
\begin{figure}[H]
    ...
    \caption{Wage Distribution for each Sector (vs MLE-estimated Log-Normal)}
...
$\ln a \in [\ln \underline a, \ln \bar a]$ with $N_a = \mNA$ points,
and a log-normal sector productivity grid $\ln \theta^s \sim N(\mu_s, \sigma_s^2)$,
that is discretized by a Gauss-Hermite quadrature with $N_\theta = \mNT$ nodes
...
The disutility of labor $\psi = \mPsi$ is internally calibrated to normalize
formal hours $h_F = \mHF$.
...
We will use $\delta_\beta = \mDbeta$ and $\omega_I = \mOmegaI$ to approximate
...
equilibrium, as shown in Table \ref{tab:transitions}. The other transition
probabilities were left untargeted. The taste preferences smoothness is governed
by $\sigma_S = \mSig$. % talvez setar \sigma_S para aproximar F -> F, I -> I, U -> U
% muitos desempregados ficam no desemprego, talvez não deixar \sigma para ele

\input{transition_table.tex}

We estimate the distribution of $\theta$ by fitting a log-normal to the observed
wage distribution via maximum likelihood, which yields $\sigma_F = \mSigmaF$ and
$\sigma_I = \mSigmaI$. We normalize average formal productivity to
$\E(\theta^F) = 1$. Finally, we set $\mu_I = \mMuI$ to match the observed
cross-sector wage gap $\xi := \E(y^F) / \E(y^I) = \mDatXi$.

The output is normalized $Y=\mY$ and we defined $\mu = \mMu$, which is standard in
the HANK literature \parencite{kaplan2018monetary,auclert2025fiscal}\footnote{
    ...
    \textcite{basu1997returns}.}, and $\kappa = \kappa_w = \mKappa$,
...
\parencite{dos2024natural}. The Government debt is set to $80\%$ of yearly GDP,
consistent with Brazilian National Treasure data. Wage tax is an average of
all contributions levied on the payroll, $\tau_\ell = \mTauL$.

The \textit{Bolsa Família} statistics were obtained from the Cadastro
Único\footnote{
...
from the Brazilian economy, with $T = \text{R\$ }670$ and $w = \text{R\$ }\mDatYF$.
We also calibrated the \textit{Bolsa Família} threshold $\bar y = \mYBar$ to
match the size of the program $BF = \mDatBF$, considering only households in
the labor force.
